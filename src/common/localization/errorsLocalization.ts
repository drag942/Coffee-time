export const errorsLocalization = {
    "ru": {
        "unknownError": "Произошла ошибка, попробуйте позже",
        "listErrorTitle": "Произошла ошибка загрузки",
        "tryAgain": "Попробовать еще раз",
        "invalidEmail": "Не верный формат почты",
        "invalidPassword": "Пароль должен содержать минимум 6 символов",
        "loginError": "Неверный логин или пароль",
        "notFound": "Элемент не найден",
        "regConfrimPassError": "Пароли не совпадают",

        "noInternetConnection": "Нет соединения с Интернетом",
        "noPhoto": "Сфотографируйте ТТН",
        "invalidWeight": "Вес должен быть числом (больше 0)",
        "error": "Ошибка: ",
        "offlineDataSendingError": "Ошибка отправки оффлайн данных (обратитесь к администратору): ",
        "runDoesntExist": "Рейс не найден",
        "notDriverError": "Авторизовываться могут только водители"
    }
};